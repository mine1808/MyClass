    ...
    inner class ItemStudentViewHolder(private val binding: ItemStudentBinding) :
        ...
        fun bind(data: Student) {
            with(binding) {
                ...
                studentSvt.setImageResource(data.resID)
                txtSvtName.text = data.name
                txtSvtNim.text = data.nim
                txtSvtGpa.text = data.gpa.toString()

                if (data.gpa == 4.0) {
                    txtSvtGpa.setTextColor(Color.GREEN)
                } else if (data.gpa >= 3.5 && data.gpa < 4.0) {
                    txtSvtGpa.setTextColor(Color.BLUE)
                } else if (data.gpa >= 3.0 && data.gpa < 3.5) {
                    txtSvtGpa.setTextColor(Color.MAGENTA)
                } else {
                    txtSvtGpa.setTextColor(Color.RED)
                }
